...
export default function EditorSlashMenu({ query, position, onSelect, onClose }: Props) {
  ...
  if (filtered.length === 0) return (
    <div
      className="absolute z-50 bg-[var(--bg-elevated)] border border-[var(--border-medium)] rounded-xl px-3 py-2.5 shadow-2xl text-xs text-[var(--text-faint)]"
  ...
  return (
    <div
      ref={menuRef}
      className="absolute z-50 bg-[var(--bg-elevated)] border border-[var(--border-medium)] rounded-xl py-1.5 shadow-2xl w-60 max-h-72 overflow-y-auto scrollbar-hide"
      style={{ top: position.top, left: position.left }}
    >
      <p className="px-3 py-1 text-[10px] font-medium uppercase tracking-widest text-[var(--text-disabled)]">
        Commands
      </p>
      {filtered.map((cmd, idx) => (
        <button
          ...
          className={`w-full flex items-center gap-3 px-3 py-2 transition-all text-left ${
            idx === activeIdx ? "bg-[var(--bg-hover)]" : "hover:bg-[var(--bg-elevated)]"
          }`}
        >
          <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 border transition-colors ${
            idx === activeIdx
              ...
              : "bg-[var(--bg-hover)] border-[var(--border-subtle)] text-[var(--text-faint)]"
          }`}>
            {cmd.icon}
          </div>
          <div className="min-w-0">
            <div className={`text-sm leading-tight transition-colors ${
              idx === activeIdx ? "text-[var(--text-primary)]" : "text-[var(--text-faint)]"
            }`}>
              {cmd.label}
            </div>
