...
/**
 * Fixed loaders:
 * - use bezier easings (typed)
 * - replaced invalid `skew` with `skewX`
 * - ensured keyframe arrays match `times`
 */
...
export const LoaderFour: React.FC<{ text?: string }> = ({ text = "Loading..." }) => {
  return (
    <div className="relative font-bold text-black [perspective:1000px] dark:text-white">
      {/* main label — use skewX instead of skew */}
      <motion.span
        animate={{
          skewX: [0, -40, 0],
          scaleX: [1, 2, 1],
      ...
      {/* purple glow layer (6 keyframes) */}
      <motion.span
        className="absolute inset-0 text-[#8b00ff]/50 dark:text-[#8b00ff]"
        animate={{
          x: [0, 1, -1.5, 1.5, -1, 0],
          y: [0, -1, 1.5, -0.5, 0, 0.2],
          opacity: [0.4, 0.8, 0.3, 0.9, 0.4, 0.6],
        }}
        transition={{
          duration: 0.8,
          repeat: Infinity,
...
export const LoaderFive: React.FC<{ text: string }> = ({ text }) => {
  return (
    <div className="font-sans font-bold [--shadow-color:var(--color-neutral-500)] dark:[--shadow-color:var(--color-neutral-100)]">
      {text.split("").map((char, i) => (
        <motion.span
          ...
          animate={{
            scale: [1, 1.1, 1],
            // textShadow isn't officially typed in framer's MotionStyle, but it works at runtime.
            // If TypeScript complains, cast the style to `any` or wrap this with a style attribute.
            textShadow: [
