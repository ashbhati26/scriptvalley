      ...
      <div className="flex flex-col min-h-[calc(100vh-3rem)]">
        ...
        <div className="sticky top-0 z-20 flex items-center justify-between gap-3 px-4 md:px-6 h-12 border-b border-(--border-subtle) bg-(--bg-base)/95 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            {/* Preview — only shown when there's something to preview */}
            {(title || modules.length > 0) && (
              <button
                onClick={() => setShowPreview(true)}
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs text-(--brand) hover:bg-(--brand-subtle) border border-(--brand-border) transition-colors"
              >
        ...
        {isPublished && (
          <div className="flex items-center gap-2.5 mx-4 md:mx-8 mt-4 px-4 py-2.5 rounded-xl border border-(--brand-border) bg-(--brand-subtle) text-(--brand) text-xs font-medium">
            <Eye className="w-3.5 h-3.5" />
            This course is live. You can edit freely — re-submit when ready for admin review.
          </div>
        )}
        ...
        <div className="flex-1 px-5 md:px-8 py-8 max-w-2xl space-y-7">
          <div>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {/* Secondary import nudge — only on empty new course */}
              {isNew && !title && modules.length === 0 && (
                <button
                  onClick={() => setShowImport(true)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-(--border-subtle) text-xs text-(--text-faint) hover:text-(--brand) hover:border-(--brand-border) hover:bg-(--brand-subtle) transition-colors"
                >
                  <FileJson className="w-3.5 h-3.5" />Import from JSON
                </button>
              )}
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex gap-1 p-1 rounded-lg bg-(--bg-input) border border-(--border-subtle)">
                {(["freeform", "structured"] as CourseTemplate[]).map((t) => (
                  ...
                  >
                    {t === "freeform"
                      ? <><FileText className="w-3 h-3" />Freeform</>
                      : <><LayoutList className="w-3 h-3" />Structured</>
                    }
                  </button>
                ))}
              </div>
