...
export default function AdminLayout() {
  ...
  type NavItem = { key: Mode; label: string; Icon: LucideIcon };
  ...
  const platformNav: NavItem[] = [
    { key: "team",           label: "Admins",         Icon: UsersRound  },
  ...
  function handleNavClick(key: Mode) {
    if (key === "reports") { setReportUserId(null); }
    setMode(key);
  }
    ...
    <div className="flex flex-col md:flex-row min-h-[calc(100vh-3rem)]">
        ...
        {[...coreNav, ...platformNav].map((it) => (
          <button key={it.key} onClick={() => handleNavClick(it.key)}
            className={`shrink-0 px-3 py-1.5 rounded-md text-xs transition-colors ${
              mode === it.key ? "bg-(--bg-active) text-(--text-primary)" : "text-text-faint hover:text-text-muted hover:bg-(--bg-hover)"
            }`}>
            {it.label}
          </button>
        ))}
      ...
      <aside className="hidden md:flex flex-col w-56 shrink-0 border-r border-(--border-subtle) sticky top-12 h-[calc(100vh-3rem)]">
        <div className="px-4 py-3 border-b border-(--border-subtle)">
          <p className="text-[10px] uppercase tracking-widest text-text-disabled mb-0.5">Dashboard</p>
          <h2 className="text-sm font-medium text-(--text-primary)">Admin Panel</h2>
        </div>

        <nav className="flex-1 overflow-y-auto px-1.5 py-2">
          <div className="space-y-px">
            {coreNav.map((it) => (
              <NavButton key={it.key} it={it} mode={mode} onClick={() => handleNavClick(it.key)} />
            ))}
          </div>

          <div className="my-3 px-2">
            <p className="text-[9px] uppercase tracking-widest text-text-disabled mb-2">Instructor Platform</p>
        ...
        <div className="px-4 py-3 border-t border-(--border-subtle)">
          <Link href="https://scriptvalley.com"
      ...
      <main className="flex-1 min-w-0 px-4 py-8 md:px-8 md:py-10">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {mode === "stats" && <PlatformStats />}
            {mode === "reports" && !reportUserId && (
              <ReportsList onSelect={(uid) => setReportUserId(uid)} />
            )}
            {mode === "reports" && reportUserId && (
              <UserReport userId={reportUserId} onBack={() => setReportUserId(null)} />
            )}
            {mode === "users"         && <UserManagement />}
            {mode === "experiences"   && (
              <div>
                <div className="mb-6">
                  <p className="text-[10px] uppercase tracking-widest text-text-disabled mb-1">Moderation</p>
                  <h2 className="text-xl font-semibold text-(--text-primary)">Interview Experiences</h2>
                  <p className="mt-1 text-sm text-text-faint">Review and publish submitted interview experiences.</p>
                </div>
                <ExperienceAdminPanel />
              </div>
            )}
            {mode === "announcements" && <AnnouncementManager />}
            {mode === "team"          && <TeamManager />}
            {mode === "instructors"   && <InstructorsList />}
            {mode === "content-review" && <ContentReview />}
            {mode === "courses"        && <CoursesList />}
            {mode === "dsa-sheets"     && <DSASheetsList />}
            {mode === "missions"       && <MissionBuilder />}
          </motion.div>
        </AnimatePresence>
      </main>
    </div>
...
function NavButton({ it, mode, onClick }: { it: { key: string; label: string; Icon: LucideIcon }; mode: string; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick}
      className={`relative w-full text-left flex items-center gap-2.5 px-2.5 py-2 rounded-md text-sm transition-colors ${
        active ? "bg-(--bg-active) text-(--text-primary)" : "text-text-muted hover:bg-(--bg-hover) hover:text-text-secondary"
      }`}
    >
      {active && (
        <motion.span layoutId="adminNav"
          className="absolute left-0 top-1.5 bottom-1.5 w-0.75 bg-[#3A5EFF] rounded-r-full" />
      )}
      <it.Icon className={`w-3.5 h-3.5 shrink-0 ${active ? "text-[#3A5EFF]" : "text-text-faint"}`} />
